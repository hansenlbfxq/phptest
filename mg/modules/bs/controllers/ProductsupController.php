...
class ProductsupController extends QrckController
{
    ...
    public function actionIndex()
    {
        ...
        $model = BsSdProductSup::find()->where('status<9');
    ...
    public function actionView()
    {
        ...
        $data = BsSdProductSup::findOne($id);
        if ($data) {
            return $data;
        } else {
     ...
     * @access public
     * @since 1.0
     * @return BsSdProductSup
     * @throws ServerErrorHttpException
     * @throws \yii\base\InvalidConfigException
     */
    public function actionCreate()
    {
        $model = new BsSdProductSup();
    ...
    public function actionUpdate()
    {
        ...
        $model = BsSdProductSup::findOne($id);
        if ($model) {
            $model->load(Yii::$app->getRequest()->getBodyParams(), '');
            if ($model->save()) {
                return $model;
    ...
    public function actionStatus()
    {
        ...
        if (empty($ids) || empty($status)) {
            ...
            $ids = explode(",", $ids);
            foreach ($ids as $i) {
                $model = BsSdProductSup::findOne($i);
